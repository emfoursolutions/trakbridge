...
def upgrade():
    ...
    if not table_exists("stream_tak_servers"):
        op.create_table(
            ...
            sa.ForeignKeyConstraint(
                ["tak_server_id"], ["tak_servers.id"], ondelete="CASCADE"
            ),
    ...
    if not index_exists("idx_stream_tak_servers_stream_id"):
        op.create_index(
            "idx_stream_tak_servers_stream_id", "stream_tak_servers", ["stream_id"]
        )
    ...
    if existing_relationships:
        ...
        print(
            f"Migrated {len(junction_data)} existing stream-server relationships to junction table"
        )
...
def downgrade():
    ...
    if index_exists("idx_stream_tak_servers_tak_server_id"):
        op.drop_index(
            "idx_stream_tak_servers_tak_server_id", table_name="stream_tak_servers"
        )

    if index_exists("idx_stream_tak_servers_stream_id"):
        op.drop_index(
            "idx_stream_tak_servers_stream_id", table_name="stream_tak_servers"
        )
