    ...
    async def test_configuration_changes_propagate_quickly(self):
        ...
        with patch('services.cot_service.PYTAK_AVAILABLE', True):
            with patch('services.cot_service.pytak.protocol_factory') as mock_factory:
                ...
                assert propagation_time < 15.0, f"Config change took {propagation_time}s, expected < 15s"
        ...
        with patch('services.cot_service.PYTAK_AVAILABLE', True):
            with patch('services.cot_service.pytak.protocol_factory') as mock_factory:
                ...
                if transmitted_batches:
                    ...
                    assert len(full_batches) >= 2, f"Expected at least 2 full batches, got {transmitted_batches}"
            ...
            with patch('services.cot_service.pytak.protocol_factory') as mock_factory:
                ...
                for i in range(200):  # 10x the queue size
                    ...
                    if i % 50 == 0:
                        ...
                        assert memory_growth < 50, f"Memory grew by {memory_growth}MB, indicating potential leak"
                
                # Final memory check
                final_memory = process.memory_info().rss / 1024 / 1024
                total_growth = final_memory - initial_memory
                
                assert total_growth < 30, f"Total memory growth {total_growth}MB too high"
        ...
        with patch('services.cot_service.PYTAK_AVAILABLE', True):
            with patch('services.cot_service.pytak.protocol_factory') as mock_factory:
                ...
                if len(transmission_times) >= 2:
                    ...
                    assert avg_interval > 0.01, "Transmissions too rapid - may overwhelm server"
    ...
    async def test_no_breaking_changes_to_existing_functionality(self):
        ...
        with patch('services.cot_service.PYTAK_AVAILABLE', True):
            with patch('services.cot_service.pytak.protocol_factory') as mock_factory:
                ...
                assert success is True or success is None  # Some implementations don't return bool
        ...
        with patch('services.cot_service.PYTAK_AVAILABLE', True):
            with patch('services.cot_service.pytak.protocol_factory') as mock_factory:
                ...
                for i in range(point_count):
                    ...
                    test_event = f'<event><point lat="{lat}" lon="{lon}"/></event>'.encode()
                    task = asyncio.create_task(
                ...
                assert total_time < processing_time_limit, f"Processing took {total_time}s, expected < {processing_time_limit}s"
