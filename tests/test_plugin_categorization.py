...
class TestPluginCategoryAPI:
    ...
    def test_get_plugin_categories_endpoint(self, client, mock_app_with_categories, auth_headers):
        ...
        expected_categories = {'OSINT', 'Tracker'}  # Based on standardized plugin categories
    ...
    def test_get_plugins_by_category_endpoint(self, client, mock_app_with_categories, auth_headers):
    ...
    def test_get_plugins_by_nonexistent_category(self, client, mock_app_with_categories, auth_headers):
        """Test getting plugins for non-existent category"""
        response = client.get('/api/plugins/by-category/NonExistent', headers=auth_headers)
    ...
    def test_get_categorized_plugins_endpoint(self, client, mock_app_with_categories, auth_headers):
    ...
    def test_get_category_statistics_endpoint(self, client, mock_app_with_categories, auth_headers):
...
class TestStreamCreationWithCategories:
    ...
    def test_create_stream_form_loads_categories(self, client, mock_app_with_categories, auth_headers):
    ...
    def test_edit_stream_form_loads_categories(self, client, mock_app_with_categories, auth_headers):
    ...
    def test_plugin_category_mapping_accuracy(self, mock_app_with_categories):
        ...
        with mock_app_with_categories.app_context():
            ...
            for plugin_key, expected_category in expected_mappings.items():
                ...
                assert actual_category == expected_category, f"Plugin {plugin_key} should be in {expected_category}, but is in {actual_category}"
    ...
    def test_all_plugins_have_categories(self, mock_app_with_categories):
        ...
        with mock_app_with_categories.app_context():
            ...
            for plugin_key in all_metadata.keys():
                ...
                assert isinstance(category, str), f"Plugin {plugin_key} category should be string, got {type(category)}"
    ...
    def test_category_data_available_in_plugin_metadata(self, mock_app_with_categories):
        ...
        with mock_app_with_categories.app_context():
            ...
            for plugin_key, metadata in all_metadata.items():
                ...
                assert isinstance(metadata['category'], str), f"Plugin {plugin_key} category should be string"
