...
from services.auth.decorators import (
    ...
    admin_required, operator_required, get_current_user
)
...
class TestLDAPAuthProvider:
    ...
    def test_ldap_authentication_success(self, mock_connection_class, mock_server_class, auth_config):
        ...
        def side_effect(*args, **kwargs):
            if 'member' in str(kwargs.get('search_filter', '')):
                mock_connection.entries = [
                    Mock(entry_dn='CN=Users,DC=test,DC=com')
                ]
            return True
    ...
    @patch('ldap3.Server')
    @patch('ldap3.Connection')
    def test_ldap_authentication_failure(self, mock_connection_class, mock_server_class, auth_config):
...
class TestAuthenticationManager:
    ...
    def test_manager_initialization(self, app, auth_config):
        ...
        with app.app_context():
            with patch('services.auth.auth_manager.load_auth_config', return_value=auth_config):
    ...
    def test_authenticate_priority_order(self, app, test_users, auth_config):
        ...
        with app.app_context():
            ...
            with patch('services.auth.auth_manager.load_auth_config', return_value=config):
                manager = AuthenticationManager()
                
                # Should try LDAP first, then fall back to local
                with patch.object(manager.ldap_provider, 'authenticate', return_value=None):
    ...
    def test_require_auth_decorator(self, app, test_users):
        ...
        with app.app_context():
            ...
            with app.test_request_context():
                ...
                # User in session
                with patch('services.auth.decorators.get_current_user', return_value=test_users['user']):
                    response = protected_view()
    ...
    def test_require_role_decorator(self, app, test_users):
        ...
        with app.app_context():
            ...
            with app.test_request_context():
                ...
                with patch('services.auth.decorators.get_current_user', return_value=test_users['admin']):
                    response = admin_view()
    ...
    def test_require_permission_decorator(self, app, test_users):
        ...
        with app.app_context():
            ...
            with app.test_request_context():
                ...
                with patch('services.auth.decorators.get_current_user', return_value=test_users['operator']):
                    response = write_streams()
    ...
    def test_role_based_access_integration(self, app, test_users):
        ...
        with app.app_context():
            ...
            for user, resource, action, expected in users_and_permissions:
                ...
                assert result == expected, f"User {user.username} should {'have' if expected else 'not have'} {action} access to {resource}"
