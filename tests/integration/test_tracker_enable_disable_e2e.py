...
class TestTrackerEnableDisableE2E:
    ...
    def test_complete_user_workflow_create_stream(self, app, db_session, client):
        """
        ...
        with app.app_context():
            ...
            tak_server = TakServer(
                name=f"E2E Test Server {uuid.uuid4()}",
                host="localhost",
                port=8087
            )
            ...
            response = client.post('/create_stream', data=create_data, follow_redirects=True)
            ...
            # E2E001 (enabled) - should be mapped
            assert mock_gps_locations[0]['name'] == 'E2E-Alpha'
            
            # E2E002 (disabled) - should be filtered out during processing
            # Note: In the actual implementation, disabled trackers are filtered before CoT generation
            
            # E2E003 (enabled) - should be mapped  
            assert mock_gps_locations[2]['name'] == 'E2E-Charlie'

            # 5. Test tracker discovery and state preservation
            with patch('services.connection_test_service.ConnectionTestService.discover_trackers') as mock_discover:
                mock_discover.return_value = {
                    ...
                    'trackers': [
                        ...
                        {'identifier': 'E2E004', 'name': 'Device 4', 'uid': 'uid004'},  # New tracker
                    ]
                }
                
                # API call to discover trackers
                response = client.post(
                    f'/api/streams/{stream.id}/discover-trackers',
    ...
    def test_edge_case_no_trackers_discovered(self, app, db_session, client):
    ...
    def test_edge_case_all_trackers_disabled(self, app, db_session):
        ...
        with app.app_context():
            ...
            tak_server = TakServer(name="All Disabled Test", host="localhost", port=8087)
            db_session.add(tak_server)
            db_session.commit()
            ...
            async def test_all_disabled():
                ...
                enabled_locations = [
                    loc for loc in all_disabled_locations
    ...
    def test_performance_with_large_tracker_counts(self, app, db_session):
        ...
        with app.app_context():
            ...
            for i, location in enumerate(large_locations):
                if i % 3 != 0:  # Should be enabled
                    assert location['name'] == f'Perf-{i:03d}'
                    enabled_count += 1
                # Disabled trackers keep original names but will be filtered in actual processing
            
            # Should have processed ~67 enabled trackers
            assert enabled_count >= 65  # Allow for small variance

    def test_multiple_gps_providers_integration(self, app, db_session):
        """Test with multiple GPS providers (Garmin, SPOT, Traccar)"""
        with app.app_context():
            tak_server = TakServer(name="Multi-Provider Test", host="localhost", port=8087)
            db_session.add(tak_server)
            db_session.commit()
            ...
            for provider, id_field in zip(providers, identifier_fields):
                ...
                stream.set_plugin_config({'username': f'{provider}_user', 'password': 'test'})
                db_session.add(stream)
                db_session.commit()
                ...
                if provider == 'garmin':
                    locations = [
                        {
                            ...
                            'lat': 40.0, 'lon': -120.0, 'uid': 'garmin-001',
                            'additional_data': {
                    ...
                    locations = [
                        {
                            'name': 'SPOT Device 1',
                            'lat': 41.0, 'lon': -121.0, 'uid': 'spot-001',
                    ...
                    locations = [
                        {
                            ...
                            'lat': 42.0, 'lon': -122.0, 'uid': 'traccar-001',
    ...
    def test_rollback_scenarios(self, app, db_session):
        ...
        with app.app_context():
            ...
            try:
                ...
                db_session.rollback()
                rollback_stream = Stream.query.filter_by(name='Rollback Test Stream').first()
                assert rollback_stream is None
                
                rollback_mappings = CallsignMapping.query.filter_by(identifier_value='ROLLBACK001').all()
            ...
            columns = [col['name'] for col in inspector.get_columns('callsign_mappings')]
...
class TestPhase6UserDocumentationAndPolish:
    ...
    def test_ui_tooltips_and_help_text(self, app, client):
        ...
        with app.app_context():
            ...
            response = client.get('/create_stream')
            assert response.status_code == 200
    ...
    def test_ui_styling_and_visual_feedback(self, app, db_session, client):
        ...
        with app.app_context():
            ...
            response = client.get(f'/edit_stream/{stream.id}')
            assert response.status_code == 200
