...
class TestDeepstateLoadScenarios:
    ...
    async def test_deepstate_300_points_sequential(self):
        ...
        with patch('services.cot_service.PYTAK_AVAILABLE', True):
            with patch('services.cot_service.pytak.protocol_factory') as mock_factory:
                ...
                assert total_time < max_processing_time, f"Processing took {total_time:.2f}s, expected < {max_processing_time}s"
                
                if transmitted_events:
                    throughput = len(transmitted_events) / total_time
                    assert throughput > 5.0, f"Throughput {throughput:.1f} events/sec too low"
                    print(f"Processed {point_count} events in {total_time:.2f}s ({throughput:.1f} events/sec)")
    ...
    async def test_deepstate_300_points_burst(self):
        ...
        with patch('services.cot_service.PYTAK_AVAILABLE', True):
            with patch('services.cot_service.pytak.protocol_factory') as mock_factory:
                ...
                assert total_time < max_processing_time, f"Burst processing took {total_time:.2f}s, expected < {max_processing_time}s"
                ...
                assert final_queue_size <= queue_config["max_size"], f"Queue exceeded max size: {final_queue_size}"
                
                print(f"Burst test: {point_count} events in {total_time:.2f}s, {transmission_count} transmitted")
    ...
    async def test_deepstate_multiple_streams_concurrent(self):
        ...
        tak_servers = [
            TakServer(id=i, name=f"deepstate-stream-{i}", host="localhost", port=8089+i)
        ...
        with patch('services.cot_service.PYTAK_AVAILABLE', True):
            with patch('services.cot_service.pytak.protocol_factory') as mock_factory:
                ...
                # Performance assertions
                assert total_time < max_processing_time, f"Concurrent processing took {total_time:.2f}s, expected < {max_processing_time}s"
                
                # Calculate overall throughput
                total_transmitted = sum(transmission_counts.values())
                if total_transmitted > 0:
                    throughput = total_transmitted / total_time
                    assert throughput > 10.0, f"Concurrent throughput {throughput:.1f} events/sec too low"
                    print(f"Concurrent test: {total_points} events across {streams} streams in {total_time:.2f}s")
        ...
        with patch('services.cot_service.PYTAK_AVAILABLE', True):
            with patch('services.cot_service.pytak.protocol_factory') as mock_factory:
                ...
                for batch in range(10):  # 10 batches of 50 events each
                    ...
                    assert memory_growth < 100, f"Memory grew by {memory_growth:.1f}MB after batch {batch}"
                ...
                assert total_growth < 50, f"Total memory growth {total_growth:.1f}MB exceeds limit"
                print(f"Memory test: processed 500 events, memory growth: {total_growth:.1f}MB")
                
                await cot_service.stop_worker(tak_server.id)

    def _create_deepstate_cot_event(self, uid: str, lat: float, lon: float, 
                                   timestamp: float, callsign: str, 
                                   speed: float = 30.0, course: float = 180.0) -> bytes:
    ...
    async def test_queue_overflow_handling_under_load(self):
        ...
        with patch('services.cot_service.PYTAK_AVAILABLE', True):
            with patch('services.cot_service.pytak.protocol_factory') as mock_factory:
                ...
                queue = cot_service.queues[tak_server.id]
                assert queue.qsize() <= small_queue_size, f"Queue size {queue.qsize()} exceeds limit {small_queue_size}"
                
                print(f"Overflow test: added {overflow_events} events, queue size: {queue.qsize()}")
        ...
        with patch('services.cot_service.PYTAK_AVAILABLE', True):
            with patch('services.cot_service.pytak.protocol_factory') as mock_factory:
                ...
                if transmission_batches:
                    ...
                    efficiency = len(full_batches) / len(transmission_batches) if transmission_batches else 0
                    
                    assert efficiency >= 0.7, f"Batching efficiency {efficiency:.2f} too low (should be >= 70%)"
                    print(f"Batch efficiency: {len(full_batches)}/{len(transmission_batches)} full batches ({efficiency:.1%})")
