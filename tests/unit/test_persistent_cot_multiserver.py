...
<event version="2.0" uid="device-{i:03d}" type="a-f-G-U-C" how="m-g" 
    ...
    def test_multi_server_event_distribution(self, persistent_cot_service, sample_cot_events, mock_tak_servers):
        """
    ...
    def test_per_server_isolation(self, persistent_cot_service, sample_cot_events, mock_tak_servers):
        """
...
<event version="2.0" uid="device-{i:03d}" type="a-f-G-U-C" how="m-g" 
    ...
    <detail>
        <contact callsign="Test-Device-{i:03d}"/>
            ...
            result = await persistent_cot_service.enqueue_with_replacement(
                newer_events,
                mock_tak_servers[1].id
            )
            
            assert result, "Second server should accept newer events with independent device state"
...
<event version="2.0" uid="test-device" type="a-f-G-U-C" how="m-g" 
    ...
    <detail>
        <contact callsign="Test-Device"/>
            ...
            await persistent_cot_service.enqueue_with_replacement(old_events, mock_tak_servers[0].id)
            
            # Send new event to same server - should replace old event
            await persistent_cot_service.enqueue_with_replacement(new_events, mock_tak_servers[0].id)
            ...
            assert queue_size_1 == 1, f"First server should have 1 event after replacement, got {queue_size_1}"
            ...
            await persistent_cot_service.enqueue_with_replacement(old_events, mock_tak_servers[1].id)
    ...
    def test_device_state_manager_per_server_access(self, persistent_cot_service, mock_tak_servers):
        """
        ...
        assert hasattr(persistent_cot_service, 'device_state_managers'), (
            "PersistentCOTService should have device_state_managers dict attribute"
        )
        
        assert isinstance(persistent_cot_service.device_state_managers, dict), (
        ...
        assert len(persistent_cot_service.device_state_managers) == 0, (
        ...
        assert server_id in persistent_cot_service.device_state_managers, (
            f"Server {server_id} should have device state manager after access"
        )
        
        assert isinstance(persistent_cot_service.device_state_managers[server_id], DeviceStateManager), (
        ...
        if server_id_2 not in persistent_cot_service.device_state_managers:
            persistent_cot_service.device_state_managers[server_id_2] = DeviceStateManager()
            
        assert (persistent_cot_service.device_state_managers[server_id] is not 
    ...
    def test_memory_usage_acceptable(self, persistent_cot_service, mock_tak_servers, sample_cot_events):
        """
        ...
        async def run_test():
            ...
            assert actual_managers == expected_servers, (
