...
def test_cot_type_mode_fix():
    ...
    with app.app_context():
        ...
        assert (
            retrieved_stream.cot_type_mode == "per_point"
        ), f"Database value incorrect: {retrieved_stream.cot_type_mode}"
        print(
            f"✅ Database query returned cot_type_mode: {retrieved_stream.cot_type_mode}"
        )
        ...
        assert (
            default_stream.cot_type_mode == "stream"
        ), f"Expected default 'stream', got '{default_stream.cot_type_mode}'"
        print(
            f"✅ Default stream created with cot_type_mode: {default_stream.cot_type_mode}"
        )

        # Test 4: Stream mode
        print("\nTest 4: Creating stream with cot_type_mode='stream'")
        stream_mode = Stream(
            name="Stream Mode Test", plugin_type="deepstate", cot_type_mode="stream"
        )
        ...
        assert (
            stream_mode.cot_type_mode == "stream"
        ), f"Expected 'stream', got '{stream_mode.cot_type_mode}'"
