...
def test_callsign_mapping_creation_conditions():
    ...
    for identifier, callsign, expected in test_cases:
        ...
        assert (
            should_create_new == expected
        ), f"Failed for ({identifier}, {callsign}): expected {expected}"
        ...
        if identifier == "TRACKER002":  # Empty callsign case
            assert (
                would_create_old != should_create_new
            ), "New logic should differ from old for empty callsign"
