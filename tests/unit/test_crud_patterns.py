...
class TestDatabaseHelperPatterns:
    ...
    def test_database_helper_crud_operations(self, app, db_session):
        ...
        with app.app_context():
            ...
            assert stream is not None, "DatabaseHelper.create should return created instance"
            assert stream.name == "Test Stream"
            assert stream.id is not None

            # Read
            found_stream = stream_helper.find_by_id(stream.id)
            assert found_stream is not None, "DatabaseHelper.find_by_id should find created stream"
            ...
            assert updated_stream is not None, "DatabaseHelper.update should return updated instance"
            assert updated_stream.name == "Updated Stream"

            # Delete
            delete_result = stream_helper.delete(stream)
            assert delete_result is True, "DatabaseHelper.delete should return True on success"
    ...
    def test_service_classes_use_database_helpers(self, app, db_session):
        ...
        with app.app_context():
            ...
            assert hasattr(service, '_stream_helper'), \
                "StreamOperationsService should use DatabaseHelper for Stream operations"
            assert hasattr(service, '_tak_server_helper'), \
            ...
            with patch.object(service, '_stream_helper') as mock_helper:
                mock_helper.create.return_value = Stream(name="Test Stream", plugin_type="garmin")
    ...
    def test_database_transaction_context_manager(self, app, db_session):
        ...
        with app.app_context():
            ...
            assert found_stream is not None, "Transaction should commit automatically on success"
    ...
    def test_get_or_create_pattern_usage(self, app, db_session):
        ...
        with app.app_context():
            ...
            assert user2.email == "test@example.com", "Should keep original email, not defaults"
...
class TestServiceDatabasePatterns:
    ...
    def test_services_avoid_direct_session_access(self, app, db_session):
        ...
        with app.app_context():
            ...
            assert hasattr(service, '_get_database_helper') or hasattr(service, 'db_helper'), \
        ...
        with app.app_context():
            ...
            try:
                ...
                if isinstance(result, dict) and 'error' in result:
                    ...
                    assert 'message' in result, "Error responses should have 'message' key"
    ...
    def test_bulk_operation_efficiency(self, app, db_session):
        ...
        with app.app_context():
            ...
            server = TakServer(name="Bulk Test Server", host="localhost", port=8089, protocol="tls")
            ...
            assert duration < 0.5, f"Bulk creation took {duration:.3f}s, should be under 0.5s"
            assert len(created_streams) == 10, "Should create all 10 streams"
            
            # Verify all streams were created
            total_bulk_streams = Stream.query.filter(Stream.name.like('Bulk Stream%')).count()
    ...
    def test_relationship_loading_optimization(self, app, db_session):
        ...
        with app.app_context():
            # Create test data
            server = TakServer(name="Relation Test Server", host="localhost", port=8089, protocol="tls")
            db_session.add(server)
            db_session.flush()
            ...
            for stream in active_streams:
                ...
                assert hasattr(stream, 'tak_server'), "Stream should have tak_server loaded"
                if stream.tak_server:
                    assert hasattr(stream.tak_server, 'name'), "TakServer should have name loaded"
...
class TestDatabaseOperationStandardization:
    ...
    def test_consistent_validation_patterns(self, app, db_session):
        ...
        with app.app_context():
            ...
            assert result is None, "Should return None for invalid data, not raise exception"
            
            # Test with valid data
            valid_stream = stream_helper.create(name="Valid Stream", plugin_type="garmin")
    ...
    def test_transaction_retry_logic(self, app, db_session):
        ...
        with app.app_context():
            ...
            assert call_count[0] == 3, f"Should retry 3 times, actually called {call_count[0]} times"
