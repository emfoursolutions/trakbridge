...
class TestMultiServerSchema:
    ...
    def test_stream_can_have_multiple_tak_servers(
        self, app_context, sample_tak_servers, sample_stream
    ):
        """
        ...
        assert hasattr(
            sample_stream, "tak_servers"
        ), "Stream should have tak_servers relationship"
        ...
        assert (
            len(sample_stream.tak_servers) == 3
        ), "Stream should be associated with 3 TAK servers"
    ...
    def test_tak_server_can_have_multiple_streams(
        self, app_context, sample_tak_servers
    ):
        """
        ...
        assert hasattr(
            server, "streams_many"
        ), "TakServer should have streams_many relationship"
        ...
        assert (
            server.streams_many.count() == 3
        ), "TAK server should be associated with 3 streams"
    ...
    def test_junction_table_exists_and_has_correct_structure(self, app_context):
        ...
        assert (
            "stream_tak_servers" in tables
        ), "Junction table 'stream_tak_servers' should exist"
        ...
        assert (
            "stream_id" in column_names
        ), "Junction table should have stream_id column"
        assert (
            "tak_server_id" in column_names
        ), "Junction table should have tak_server_id column"
        ...
        assert (
            "tak_servers" in fk_tables
        ), "Junction table should have FK to tak_servers table"
    ...
    def test_backward_compatibility_maintained(self, app_context, sample_tak_servers):
        ...
        ), "Legacy tak_server relationship should still work"
    ...
    def test_migration_preserves_existing_data(self, app_context, sample_tak_servers):
        ...
        assert (
            stream.tak_server_id == original_server_id
        ), "Legacy tak_server_id should be preserved"
        assert (
            stream.tak_server is not None
        ), "Legacy tak_server relationship should work"

        # 2. Also be available via new many-to-many relationship
        # (This will be populated by migration script)
        assert hasattr(
            stream, "tak_servers"
        ), "New tak_servers relationship should exist"

    def test_cascade_behavior_for_multi_server_relationships(
        self, app_context, sample_tak_servers
    ):
        """
        ...
        assert (
            result == 0
        ), "Junction table entries should be deleted when stream is deleted"

        # TAK servers should still exist (no cascade delete to servers)
        remaining_servers = db.session.query(TakServer).count()
        assert (
            remaining_servers == 3
        ), "TAK servers should not be deleted when stream is deleted"

    def test_unique_constraints_on_junction_table(
        self, app_context, sample_tak_servers, sample_stream
    ):
        """
        ...
        try:
            ...
            assert (
                associations == 1
            ), "Should not create duplicate stream-server associations"
            ...
            assert (
                len(sample_stream.tak_servers) == 1
            ), "Original association should be preserved"
    ...
    def test_query_performance_with_many_to_many(self, app_context, sample_tak_servers):
        ...
        streams_data = [
            ...
            (
                "Stream D",
                ["Primary TAK Server", "Backup TAK Server", "Training TAK Server"],
            ),
        ]
        ...
        primary_server = next(
            s for s in sample_tak_servers if s.name == "Primary TAK Server"
        )

        streams_using_primary = (
            db.session.query(Stream)
            .join(Stream.tak_servers)
            .filter(TakServer.name == "Primary TAK Server")
            .all()
        )

        assert (
            len(streams_using_primary) == 3
        ), "Should find 3 streams using Primary TAK Server"
    ...
    def test_database_indexes_for_junction_table(self, app_context):
        ...
        assert (
            "stream_id" in index_columns
        ), "Junction table should have index on stream_id"
        assert (
            "tak_server_id" in index_columns
        ), "Junction table should have index on tak_server_id"
