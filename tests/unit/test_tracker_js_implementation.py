...
class TestJavaScriptImplementation:
    ...
    def test_create_template_has_enabled_column_header(self):
        ...
        template_path = (
            Path(__file__).parent.parent.parent / "templates" / "create_stream.html"
        )
        ...
        assert (
            "<th>Enabled</th>" in content
        ), "Enabled column header missing from create template"
        ...
        assert (
            enabled_pos < identifier_pos
        ), "Enabled column should come before Identifier column"

    def test_edit_template_has_enabled_column_header(self):
        """Verify edit_stream.html has Enabled column header"""
        template_path = (
            Path(__file__).parent.parent.parent / "templates" / "edit_stream.html"
        )
        ...
        assert (
            "<th>Enabled</th>" in content
        ), "Enabled column header missing from edit template"
    ...
    def test_toggle_tracker_enabled_function_exists_create(self):
        ...
        assert (
            "function toggleTrackerEnabled(checkbox, index)" in content
        ), "toggleTrackerEnabled function missing"
    ...
    def test_bulk_enable_trackers_function_exists_create(self):
        ...
        # Check function definition
        assert (
            "function bulkEnableTrackers(enabled)" in content
        ), "bulkEnableTrackers function missing"

        # Check function logic
        assert (
            "querySelectorAll('input[name*=\"_enabled\"]')" in content
        ), "Checkbox selection missing"
    ...
    def test_get_tracker_form_data_function_exists_create(self):
        ...
        # Check function definition
        assert (
            "function getTrackerFormData()" in content
        ), "getTrackerFormData function missing"
    ...
    def test_checkbox_elements_in_table_row_create(self):
        ...
        populate_func = content[
            content.find("function populateTrackerMappingTable") : content.find(
        ...
        assert (
            "callsign_mapping_${index}_enabled" in populate_func
        ), "Checkbox name missing"
        assert (
            'onchange="toggleTrackerEnabled(this, ${index})"' in populate_func
    ...
    def test_checkbox_elements_in_table_row_edit(self):
        ...
        assert (
            "${enabled ? 'checked' : ''}" in populate_func
        ), "Conditional checked state missing"
    ...
    def test_input_field_ids_for_interaction_create(self):
        ...
        assert (
            'id="callsign_input_${index}"' in populate_func
        ), "Callsign input ID missing"
    ...
    def test_state_persistence_functionality_create(self):
        ...
        assert (
            "function populateTrackerMappingTableWithState" in content
        ), "State persistence function missing"
        assert (
            "currentState[identifierInput.value]" in content
        ), "State storage logic missing"
        assert (
            "toggleTrackerEnabled(checkbox, index)" in content
    ...
    def test_state_persistence_functionality_edit(self):
        ...
        # Check for state persistence function
        assert (
            "function populateTrackerMappingTableWithExistingAndState" in content
        ), "State persistence function missing"
        assert (
            "currentState[identifierInput.value]" in content
        ), "State storage logic missing"
...
class TestTemplateStructureValidation:
    ...
    def test_create_template_has_phase_3_functionality(self):
        ...
        assert (
            "Phase 3: Tracker Enable/Disable Functionality" in content
        ), "Phase 3 marker missing"
    ...
    def test_javascript_syntax_basic_validation_create(self):
        ...
        assert js_content.count("{") == js_content.count(
            "}"
        ), "Mismatched JavaScript braces"
        assert js_content.count("(") == js_content.count(
            ")"
        ), "Mismatched JavaScript parentheses"
    ...
    def test_required_css_classes_and_styling(self):
        ...
        for template_path in [create_path, edit_path]:
            ...
            assert (
                "text-muted" in content
            ), f"text-muted class missing in {template_path.name}"
            assert (
                "table-secondary" in content
            ), f"table-secondary class missing in {template_path.name}"
            assert (
                "form-check" in content
            ), f"form-check class missing in {template_path.name}"
            assert (
                "form-check-input" in content
            ), f"form-check-input class missing in {template_path.name}"
