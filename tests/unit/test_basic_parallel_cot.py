...
class TestBasicParallelCOT:
    ...
    async def test_parallel_processing_300_points_faster_than_serial(
        ...
        print(f"Performance ratio: {improvement_ratio:.2f}x (serial: {serial_time:.3f}s, parallel: {parallel_time:.3f}s)")
    ...
    async def test_parallel_processing_uses_asyncio_gather(self, cot_service):
        ...
        cot_service.parallel_config["batch_size_threshold"] = 5  # Force parallel processing

        try:
            # Mock asyncio.gather at the module level where it's actually called
            with patch("services.cot_service_integration.asyncio.gather") as mock_gather:
                ...
                async def mock_gather_impl(*tasks, return_exceptions=True):
                    ...
                    return [[b"<event>chunk1</event>"] * 5, [b"<event>chunk2</event>"] * 5]
                ...
                assert all(isinstance(event, bytes) for event in result), "All results should be bytes"
