...
<event version="2.0" uid="device-{i:03d}" type="a-f-G-U-C" time="{base_time.isoformat()}" start="{base_time.isoformat()}" stale="{(base_time + timedelta(minutes=5)).isoformat()}" how="m-g">
    ...
    def test_extract_uid_from_cot_event_method_exists(self, cot_service):
        ...
        """
        assert hasattr(
            cot_service, "extract_uid_from_cot_event"
        ), "Should have extract_uid_from_cot_event method"
        assert callable(
            cot_service.extract_uid_from_cot_event
        ), "extract_uid_from_cot_event should be callable"

    def test_extract_uid_from_cot_event_extracts_correctly(
        self, cot_service, sample_cot_events
    ):
        """
    ...
    def test_extract_timestamp_from_cot_event_method_exists(self, cot_service):
        ...
        """
        assert hasattr(
            cot_service, "extract_timestamp_from_cot_event"
        ), "Should have extract_timestamp_from_cot_event method"
        assert callable(
            cot_service.extract_timestamp_from_cot_event
        ), "extract_timestamp_from_cot_event should be callable"

    def test_extract_timestamp_from_cot_event_extracts_correctly(
        self, cot_service, sample_cot_events
    ):
        """
    ...
    async def test_enqueue_with_replacement_method_exists(self, cot_service):
        ...
        """
        assert hasattr(
            cot_service, "enqueue_with_replacement"
    ...
    @pytest.mark.asyncio
    async def test_enqueue_with_replacement_adds_new_events(
        self, cot_service, sample_cot_events
    ):
        """
    ...
    async def test_enqueue_with_replacement_replaces_old_events(
        self, cot_service, sample_cot_events, updated_cot_events
    ):
        """
        ...
        cot_service.extract_uid_from_cot_event = Mock(
            side_effect=lambda e: f"device-{len(e) % 3:03d}"
        )
        cot_service.extract_timestamp_from_cot_event = Mock(
            return_value=datetime.now(timezone.utc)
        )
    ...
    def test_remove_events_by_uid_method_exists(self, cot_service):
        ...
        """
        assert hasattr(
            cot_service, "remove_events_by_uid"
    ...
    @pytest.mark.asyncio
    async def test_remove_events_by_uid_removes_matching_events(
        self, cot_service, sample_cot_events
    ):
        """
    ...
    @pytest.mark.asyncio
    async def test_enqueue_with_replacement_handles_mixed_new_and_updated_events(
        self, cot_service
    ):
        """
...
<event version="2.0" uid="device-000" type="a-f-G-U-C" time="{base_time.isoformat()}" start="{base_time.isoformat()}" stale="{(base_time + timedelta(minutes=5)).isoformat()}" how="m-g">
    <point lat="55.0" lon="-85.0" hae="5.0" ce="1.0" le="1.0"/>
        ...
        def mock_should_update(uid, timestamp):
            return (
                uid == "new-device-999" or uid == "device-000"
...
<event version="2.0" uid="device-000" type="a-f-G-U-C" time="{old_time.isoformat()}" start="{old_time.isoformat()}" stale="{(old_time + timedelta(minutes=5)).isoformat()}" how="m-g">
    ...
    async def test_enqueue_with_replacement_queue_size_stabilization(
        ...
        cot_service.remove_events_by_uid = AsyncMock(
            return_value=300
        )  # Remove 300 old events
