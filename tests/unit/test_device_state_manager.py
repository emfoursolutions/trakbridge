...
class TestDeviceStateManager:
    ...
    def test_device_state_manager_initialization(self, device_manager):
        ...
        """
        assert hasattr(device_manager, 'device_states'), "Should have device_states attribute"
        assert isinstance(device_manager.device_states, dict), "device_states should be a dictionary"
        assert len(device_manager.device_states) == 0, "Should start with empty device states"
    ...
    def test_should_update_device_method_exists(self, device_manager):
        ...
        """
        assert hasattr(device_manager, 'should_update_device'), "Should have should_update_device method"
        assert callable(device_manager.should_update_device), "should_update_device should be callable"
    ...
    def test_should_update_device_newer_timestamp(self, device_manager, sample_device_data):
        """
        ...
        device_manager.update_device_state(device_data["uid"], {
    ...
    def test_should_update_device_older_timestamp(self, device_manager, sample_device_data):
        """
    ...
    def test_update_device_state_method_exists(self, device_manager):
        ...
        STATUS: WILL FAIL - method doesn't exist
        """
        assert hasattr(device_manager, 'update_device_state'), "Should have update_device_state method"
        assert callable(device_manager.update_device_state), "update_device_state should be callable"
    ...
    def test_update_device_state_stores_data(self, device_manager, sample_device_data):
        ...
        assert device_data["uid"] in device_manager.device_states, "Device UID should be stored"
        stored_data = device_manager.device_states[device_data["uid"]]
        assert stored_data["timestamp"] == device_data["timestamp"], "Timestamp should match"
    ...
    def test_update_device_state_replaces_old_data(self, device_manager, sample_device_data):
        """
        ...
        old_timestamp = device_data["timestamp"] - timedelta(minutes=5)
        device_manager.update_device_state(uid, {
            "timestamp": old_timestamp,
            "lat": old_lat,
            "lon": old_lon
        })
        ...
        assert stored_data["timestamp"] == device_data["timestamp"], "Should have new timestamp"
    ...
    def test_get_stale_devices_method_exists(self, device_manager):
        ...
        STATUS: WILL FAIL - method doesn't exist
        """
        assert hasattr(device_manager, 'get_stale_devices'), "Should have get_stale_devices method"
        assert callable(device_manager.get_stale_devices), "get_stale_devices should be callable"

    def test_get_stale_devices_finds_old_devices(self, device_manager, sample_device_data):
        """
        ...
        assert len(stale_devices) == 1, f"Should find 1 stale device, found {len(stale_devices)}"
        assert sample_device_data["device_3"]["uid"] in stale_devices, "device_3 should be stale"
        assert sample_device_data["device_2"]["uid"] not in stale_devices, "device_2 should not be stale (only 5 min old)"
        assert sample_device_data["device_1"]["uid"] not in stale_devices, "device_1 should not be stale"

    def test_get_stale_devices_empty_when_all_fresh(self, device_manager, sample_device_data):
        """
        ...
        assert len(stale_devices) == 0, "Should find no stale devices when all are fresh"
    ...
    def test_multiple_plugins_device_tracking(self, device_manager, sample_device_data):
        ...
        assert len(device_manager.device_states) == 3, "Should track devices from multiple plugins"
        
        # Verify plugin sources are preserved
        deepstate_device = device_manager.device_states[sample_device_data["device_1"]["uid"]]
        garmin_device = device_manager.device_states[sample_device_data["device_2"]["uid"]]
        spot_device = device_manager.device_states[sample_device_data["device_3"]["uid"]]
