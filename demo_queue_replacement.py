...
def create_sample_cot_event(uid: str, lat: float, lon: float, timestamp: datetime = None) -> bytes:
...
async def demonstrate_queue_replacement():
    ...
    device_manager.update_device_state("device-001", {
        "timestamp": now,
        "lat": 40.7589,
        "lon": -73.9851,
        "plugin_source": "demo"
    ...
    device_manager.update_device_state("device-002", {
    ...
    updated_events = [
        create_sample_cot_event("deepstate-device-001", 40.7800, -73.9700, updated_time),  # Moved
        create_sample_cot_event("deepstate-device-002", 34.0600, -118.2300, updated_time),  # Moved
        create_sample_cot_event("deepstate-device-003", 51.5200, -0.1100, updated_time)   # Moved
    ]
    ...
    print(f"   • Event accumulation: {'❌ PREVENTED' if mock_queue.qsize() == 3 else '⚠️  DETECTED'}")
    
    # Test with older events (should be rejected)
    old_time = base_time - timedelta(minutes=5)
    old_events = [
        create_sample_cot_event("deepstate-device-001", 40.7000, -74.0000, old_time)  # Older position
    ]
    ...
    print(f"✅ Queue size before: {queue_size_before_old}, after: {queue_size_after_old}")
    print(f"✅ Old event rejected: {'✅ YES' if queue_size_after_old == queue_size_before_old else '❌ NO'}")
