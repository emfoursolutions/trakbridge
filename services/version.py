...
class VersionInfo:
    ...
    def _get_base_version(self) -> Dict[str, Any]:
        ...
        for source_func in version_sources:
            try:
                ...
                if version_info:
                    logger.debug(
                        f"Version loaded from {version_info['source']}: {version_info['version']}"
                    )
    ...
    def _get_developer_info(self) -> Dict[str, Any]:
        ...
        dev_info = {"name": "unknown", "email": "unknown"}
    ...
    def is_development_build(self) -> bool:
        ...
        self._is_development = (
            git_info["is_dirty"]
            or (git_info["branch"] and git_info["branch"] not in ["main", "master"])
            or "dev" in base_version["version"].lower()
            or env_info["debug_mode"]
            or base_version["source"] in ["fallback", "git-tag"]
        )
...
if __name__ == "__main__":
    ...
    formatted_version = version_info.format_version(
        include_git=True, include_env=True, include_build_info=True
    )
