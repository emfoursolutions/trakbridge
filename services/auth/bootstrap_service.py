...
class BootstrapService:
    ...
    def should_create_initial_admin(self) -> bool:
        ...
        try:
            ...
            if self._is_bootstrap_completed():
                logger.debug("Bootstrap already completed, skipping initial admin creation")
                return False
            
            # Check if any admin users exist
            admin_count = User.query.filter_by(role=UserRole.ADMIN).count()
            if admin_count > 0:
                logger.debug(f"Found {admin_count} existing admin users, skipping bootstrap")
                # Mark bootstrap as completed even if we didn't create the user
                self._mark_bootstrap_completed()
                return False
            
            logger.info("No admin users found and bootstrap not completed - initial admin creation required")
            return True
    ...
    def create_initial_admin(self) -> Optional[User]:
        ...
        try:
            ...
            existing_user = User.query.filter_by(username=self.default_admin_username).first()
            if existing_user:
                logger.warning(f"User '{self.default_admin_username}' already exists, cannot create initial admin")
                return None
            ...
            logger.warning(f"SECURITY: Initial admin user '{self.default_admin_username}' created automatically")
            logger.warning(f"SECURITY: Default password is '{self.default_admin_password}' - MUST be changed on first login")
            logger.info(f"Bootstrap completed successfully - admin user ID: {admin_user.id}")
    ...
    def force_password_change_required(self, user: User) -> bool:
        ...
        try:
            ...
            if (user.username == self.default_admin_username and 
