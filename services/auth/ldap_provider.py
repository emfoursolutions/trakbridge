...
class LDAPAuthProvider(BaseAuthenticationProvider):
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"LDAP server: {self.host}:{self.port} (SSL: {self.use_ssl}, TLS: {self.use_tls})")
            logger.error(f"LDAP bind password configured: {'Yes' if self.bind_password else 'No'}")
            logger.error(f"LDAP bind password length: {len(self.bind_password) if self.bind_password else 0} chars")
            return {
                'status': 'unhealthy',
                'provider': 'ldap',
                'server': f"{self.host}:{self.port}",
        ...
        for group in groups:
            ...
            if group in self.group_mappings:
                try:
                    ...
                    logger.warning(f"Invalid role mapping for group {group}: {self.group_mappings[group]}")
                    continue
