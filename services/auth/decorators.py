    ...
    def decorator(f: Callable) -> Callable:
        ...
        def decorated_function(*args, **kwargs):
            ...
            if not has_permission:
                logger.warning(f"Insufficient permissions for user {user.username} to access {request.endpoint}")
...
def api_key_or_auth_required(f: Callable) -> Callable:
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not user:
            logger.warning(f"Unauthenticated API access attempt to {request.endpoint} from {request.remote_addr}")
