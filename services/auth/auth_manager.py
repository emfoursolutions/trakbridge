...
class AuthenticationManager:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        if 'authentication' in self.config:
            ...
            self.cleanup_interval = session_config.get('cleanup_interval_minutes', 60) / 60  # Convert to hours
        else:
        ...
        try:
            ...
            for provider_config in provider_list:
                ...
                try:
                    ...
                    logger.info(f"Initializing {provider_type} provider with config: {provider_config.get('config', {})}")
            ...
            if not self.providers:
                logger.warning("No authentication providers configured, enabling local provider as fallback")
            ...
            try:
                ...
                logger.critical(f"Failed to create emergency authentication provider: {emergency_error}")
    ...
    def authenticate(self, username: str, password: str = None, 
                    provider_hint: AuthProvider = None, **kwargs) -> AuthenticationResponse:
        """
        ...
        for provider_type in providers_to_try:
            ...
            try:
                logger.info(f"Attempting authentication with {provider_type.value} for user: {username}")
                ...
                if response.success:
                    ...
                    self._log_authentication_event(
                        username, provider_type, True, 
                        f"Successful authentication with {provider_type.value}"
                    )
                    
                    logger.info(f"Successful authentication for {username} using {provider_type.value}")
                    return response
                ...
                logger.warning(f"Authentication failed for {username} with {provider_type.value}: {response.message}")
                return response
                ...
                logger.error(f"Unexpected error with {provider_type.value} for {username}: {e}", exc_info=True)
                last_error = AuthenticationResponse(
        ...
        logger.warning(f"Authentication failed for {username} after trying providers: {attempted_providers}")
    ...
    def create_session(self, user: User, provider_type: AuthProvider = None, 
                      request_info: Dict[str, Any] = None) -> UserSession:
        """
    ...
    def get_authentication_stats(self) -> Dict[str, Any]:
        ...
        for provider_type in AuthProvider:
            count = db.session.query(UserSession).join(User).filter(
    ...
    def _is_provider_healthy(self, provider_type: AuthProvider) -> bool:
        ...
        # Check cache
        if (provider_type in self._health_cache and 
            provider_type in self._last_health_check and
            now - self._last_health_check[provider_type] < self._health_cache_ttl):
    ...
    def _log_authentication_event(self, username: str, provider_type: AuthProvider = None,
                                success: bool = False, message: str = None) -> None:
