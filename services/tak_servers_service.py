...
class TakServerService:
    ...
    def validate_certificate_data(cert_data, password):
        ...
        try:
            ...
            is_not_yet_valid = (
                certificate.not_valid_before.replace(tzinfo=timezone.utc) > now
            )
            ...
            return {
                "success": False,
                "error": f"Certificate validation failed: {error_msg}",
            }
    ...
    def validate_stored_certificate(server):
        ...
        try:
            ...
            try:
                ...
                password = (
                    server.get_cert_password().encode("utf-8")
                    if server.get_cert_password()
                    else None
                )

                private_key, certificate, additional_certificates = (
                    pkcs12.load_key_and_certificates(cert_data, password)
                )
                ...
                try:
                    private_key, certificate, additional_certificates = (
                        pkcs12.load_key_and_certificates(cert_data, None)
                    )
                except Exception:
                    return {
                        "success": False,
            ...
            if certificate:
                ...
                cert_info["not_before"] = certificate.not_valid_before_utc.strftime(
                    "%Y-%m-%d %H:%M:%S UTC"
                )
            ...
            return {"success": True, "cert_info": cert_info}
    ...
    async def test_connection_with_timeout(config, timeout=10):
        ...
        try:
            ...
            connection_task = asyncio.create_task(
                TakServerConnectionTester.attempt_connection(config)
            )
        ...
        try:
            ...
            try:
                ...
                return {
                    ...
                    "details": {
                        "protocol": (
                            config.get("COT_URL", "").split("://")[0]
                            if "://" in config.get("COT_URL", "")
                            else "unknown"
                        ),
