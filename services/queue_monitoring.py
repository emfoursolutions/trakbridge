...
class QueueMonitoringService:
    ...
    async def _collect_and_analyze_metrics(self):
        ...
        try:
            ...
            for queue_id, status in queue_statuses.items():
                ...
                metrics = await self._calculate_queue_metrics(queue_id, status, current_time)
    ...
    async def _calculate_queue_metrics(self, queue_id: int, status: Dict[str, Any], timestamp: datetime) -> QueueHealthMetrics:
        ...
        try:
            ...
            average_wait_time = self._estimate_wait_time(current_size, events_per_second)
            ...
            trend_direction = self._calculate_trend_direction(queue_id, utilization_percent)
    ...
    def _calculate_event_rate(self, queue_id: int, status: Dict[str, Any], timestamp: datetime) -> float:
        ...
        try:
            ...
            recent_metrics = [m for m in history if (timestamp - m.timestamp).total_seconds() <= 60]
    ...
    def _calculate_batch_rate(self, queue_id: int, status: Dict[str, Any], timestamp: datetime) -> float:
    ...
    def _calculate_health_score(self, utilization: float, overflow_rate: float, wait_time: float) -> float:
    ...
    def _calculate_trend_direction(self, queue_id: int, current_utilization: float) -> str:
        ...
        try:
            ...
            history = self.metrics_history[queue_id]
            sample_points = self.config.get("trend_analysis", {}).get("sample_points", 6)
            
            if len(history) < sample_points:
                return "stable"
            
            # Get recent utilization values
            recent_utilizations = [m.utilization_percent for m in list(history)[-sample_points:]]
            
            # Calculate trend
            if len(recent_utilizations) >= 3:
                first_third = sum(recent_utilizations[:len(recent_utilizations)//3]) / (len(recent_utilizations)//3)
                last_third = sum(recent_utilizations[-len(recent_utilizations)//3:]) / (len(recent_utilizations)//3)
                
                change_percent = abs(last_third - first_third) / max(first_third, 1)
                significant_threshold = self.config.get("trend_analysis", {}).get("significant_change_percent", 20) / 100
            ...
            logger.debug(f"Failed to calculate trend direction for queue {queue_id}: {e}")
            return "stable"
    ...
    async def _analyze_queue_health(self, metrics: QueueHealthMetrics):
        ...
        try:
            ...
            if metrics.utilization_percent >= thresholds.get("utilization_critical", 95):
                alerts.append(self._create_alert(
                    metrics.queue_id, "threshold", "critical",
                    f"Queue utilization critical: {metrics.utilization_percent:.1f}%",
                    {"utilization": metrics.utilization_percent}
                ))
            elif metrics.utilization_percent >= thresholds.get("utilization_warning", 80):
                alerts.append(self._create_alert(
                    metrics.queue_id, "threshold", "warning",
            ...
            logger.error(f"Failed to analyze queue health for queue {metrics.queue_id}: {e}")

    def _create_alert(self, queue_id: int, alert_type: str, severity: str, message: str, metrics: Dict[str, Any]) -> PerformanceAlert:
    ...
    async def _process_alert(self, alert: PerformanceAlert):
        ...
        try:
            ...
            if cooldown_key in self.alert_cooldowns:
                time_since_last = datetime.now(timezone.utc) - self.alert_cooldowns[cooldown_key]
    ...
    def get_metrics_history(self, queue_id: int, hours: int = 1) -> List[QueueHealthMetrics]:
        ...
        return [
            m for m in self.metrics_history[queue_id]
...
def get_queue_monitoring_service(config: Optional[Dict[str, Any]] = None) -> QueueMonitoringService:
    """
