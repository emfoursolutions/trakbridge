...
@click.option('--format', 'output_format', type=click.Choice(['json', 'yaml', 'env', 'toml']),
...
def check(version_string, strict):
    ...
    if match:
        ...
        if strict:
            ...
            click.echo("Expected format: MAJOR.MINOR.PATCH[.ADDITIONAL][-PRERELEASE][+BUILD][.dirty|.devN]")
        sys.exit(1)
...
def debug():
    ...
    click.echo(f"Selected base version: {click.style(base_version['version'], fg='green', bold=True)}")
    click.echo(f"Version source: {base_version['source']}")
    click.echo(f"Development version: {click.style(version_instance.get_development_version(), fg='blue', bold=True)}")
    click.echo(f"Is development build: {click.style(str(version_instance.is_development_build()), fg='yellow')}")
    ...
    if git_info['available']:
        ...
        click.echo(f"  Dirty: {click.style(str(git_info.get('is_dirty', False)), fg='red' if git_info.get('is_dirty') else 'green')}")
