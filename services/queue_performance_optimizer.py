...
class QueuePerformanceOptimizer:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.performance_regression_threshold = 0.05  # 5% regression threshold
        
        # System monitoring
        self.system_process = psutil.Process()
        
        logger.info(f"QueuePerformanceOptimizer initialized with strategy: {self.strategy}")
    ...
    async def _collect_baseline_metrics(self):
        ...
        try:
            ...
            baseline_samples = []
            sample_count = self.config.get("validation", {}).get("performance_baseline_samples", 10)
            ...
            if baseline_samples:
                self.baseline_metrics = PerformanceMetrics(
                    ...
                    cpu_usage_percent=sum(m.cpu_usage_percent for m in baseline_samples) / len(baseline_samples),
                    memory_usage_mb=sum(m.memory_usage_mb for m in baseline_samples) / len(baseline_samples),
                    queue_throughput=sum(m.queue_throughput for m in baseline_samples) / len(baseline_samples),
                    batch_efficiency=sum(m.batch_efficiency for m in baseline_samples) / len(baseline_samples),
                    network_latency_ms=sum(m.network_latency_ms for m in baseline_samples) / len(baseline_samples),
                    processing_latency_ms=sum(m.processing_latency_ms for m in baseline_samples) / len(baseline_samples),
                    cache_hit_ratio=sum(m.cache_hit_ratio for m in baseline_samples) / len(baseline_samples),
                )
                
                logger.info(f"Baseline metrics established: CPU={self.baseline_metrics.cpu_usage_percent:.1f}%, "
    ...
    async def _optimize_batch_sizes(self, metrics: PerformanceMetrics):
        ...
        try:
            ...
            logger.debug(f"Optimized batch size to {target_batch_size} based on CPU usage {metrics.cpu_usage_percent:.1f}%")
    ...
    async def _optimize_memory_usage(self, metrics: PerformanceMetrics):
        ...
        try:
            if metrics.memory_usage_mb > self.strategy.target_memory_threshold_mb:
                ...
                logger.debug(f"Performed memory optimization, usage: {metrics.memory_usage_mb:.1f}MB")
    ...
    async def _cleanup_caches(self):
        ...
        keys_to_remove = [
            key for key, value in self.event_cache.items()
            if isinstance(value, dict) and value.get("timestamp", datetime.min.replace(tzinfo=timezone.utc)) < cutoff_time
        ]
        
        for key in keys_to_remove:
            del self.event_cache[key]
        
        logger.debug(f"Cleaned cache: removed {len(keys_to_remove)} entries, {cache_size_before} -> {len(self.event_cache)}")
    ...
    async def _optimize_prefetching(self, metrics: PerformanceMetrics):
        ...
        try:
            ...
            if metrics.cache_hit_ratio < 0.6:
                ...
                logger.debug("Increasing prefetch buffer size due to low cache hit ratio")
            elif metrics.cache_hit_ratio > 0.9:
                # High hit ratio, reduce prefetching to save memory
                logger.debug("Reducing prefetch buffer size due to high cache hit ratio")
    ...
    async def _optimize_caching(self, metrics: PerformanceMetrics):
        ...
        try:
            ...
            if metrics.cache_hit_ratio > 0.8 and metrics.memory_usage_mb < self.strategy.target_memory_threshold_mb * 0.8:
    ...
    async def _validate_performance(self, metrics: PerformanceMetrics):
        ...
        try:
            ...
            cpu_increase = metrics.cpu_usage_percent - self.baseline_metrics.cpu_usage_percent
            if cpu_increase > self.performance_regression_threshold * 100:  # 5% increase
                regression_detected = True
                logger.warning(f"CPU usage regression detected: {cpu_increase:.1f}% increase")
            
            # Memory usage should not increase significantly
            memory_increase = (metrics.memory_usage_mb - self.baseline_metrics.memory_usage_mb) / self.baseline_metrics.memory_usage_mb
            if memory_increase > self.performance_regression_threshold:
                regression_detected = True
                logger.warning(f"Memory usage regression detected: {memory_increase*100:.1f}% increase")
            
            # Throughput should not decrease significantly
            throughput_decrease = (self.baseline_metrics.queue_throughput - metrics.queue_throughput) / self.baseline_metrics.queue_throughput
            if throughput_decrease > self.performance_regression_threshold:
                regression_detected = True
                logger.warning(f"Throughput regression detected: {throughput_decrease*100:.1f}% decrease")
        ...
        try:
            ...
            report = {
                ...
                "current_settings": {
                    ...
                    "prefetch_buffers": {k: len(v) for k, v in self.prefetch_buffers.items()},
                },
            }
            ...
            if self.baseline_metrics:
                report["baseline_comparison"] = {
                    "cpu_improvement": self.baseline_metrics.cpu_usage_percent - latest_metrics.cpu_usage_percent,
                    "memory_change": latest_metrics.memory_usage_mb - self.baseline_metrics.memory_usage_mb,
                    "throughput_improvement": latest_metrics.queue_throughput - self.baseline_metrics.queue_throughput,
                }
        ...
        try:
            for check_function in self.test_compliance_checks:
                try:
                    ...
                    if not result:
                        logger.error(f"Test compliance check failed: {check_function.__name__}")
                        return False
                except Exception as e:
                    logger.error(f"Test compliance check error: {check_function.__name__}: {e}")
                    return False
...
def get_performance_optimizer(config: Optional[Dict[str, Any]] = None) -> QueuePerformanceOptimizer:
    """
