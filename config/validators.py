...
class ConfigValidator:
    ...
    def _validate_network_config(self, config):
        ...
        if http_max_conn_per_host > http_max_conn:
            self.warnings.append("HTTP_MAX_CONNECTIONS_PER_HOST should not exceed HTTP_MAX_CONNECTIONS")
    ...
    def _validate_production_config(self, config):
        ...
        if config.SQLALCHEMY_RECORD_QUERIES:
            self.warnings.append("SQLALCHEMY_RECORD_QUERIES should be False in production for performance")
...
class ConfigTypeChecker:
    ...
    @staticmethod
    def validate_integer(value: Any, field_name: str, min_value: int = None, max_value: int = None) -> Optional[str]:
