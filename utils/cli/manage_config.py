...
class ConfigManager:
    ...
    def __init__(self, external_config_dir: str = None):
        self.external_config_dir = Path(
            external_config_dir or os.environ.get("TRAKBRIDGE_CONFIG_DIR", "./config")
        )
            ...
            try:
                if external_file.exists():
                    if update_mode == "preserve":
                        ...
                        self.logger.info(
                            f"Preserving existing config (merge not yet implemented): {bundled_file.name}"
                        )
                        skipped_count += 1
                        continue
                else:
                ...
                if not auth_config:
                    ...
                    if not enabled_providers:
                        ...
                        results["warnings"].append(
                            f"Enabled auth providers: {', '.join(enabled_providers)}"
                        )
                        ...
                        for field in required_ldap_fields:
                            if not ldap_config.get(field):
                                results["issues"].append(
                                    f"LDAP provider missing required field: {field}"
                                )
                                results["valid"] = False
                    ...
                    if providers.get("oidc", {}).get("enabled", False):
                        ...
                        for field in required_oidc_fields:
                            if not oidc_config.get(field):
                                results["issues"].append(
                                    f"OIDC provider missing required field: {field}"
                                )
                                results["valid"] = False
...
Examples:
  ...
  python manage_config.py restore --backup-dir ./config-backup-20250127_143022
    ...
    validate_parser.add_argument(
        "--environment", default="development", help="Environment to validate"
    )
    validate_parser.add_argument(
    ...
    try:
        ...
        if args.command == "list":
            ...
            success = config_manager.install_config(
                filename=args.file, update_mode=args.update_mode
            )
