...
class ConfigManager:
    ...
    def __init__(self, external_config_dir: str = "external_config", 
                ...
                "properties": {
                    "authentication": {
                        ...
                        "properties": {
                            "provider_priority": {
                                "type": "array",
                                "items": {"type": "string", "enum": ["local", "ldap", "oidc"]}
                            },
    ...
    def load_config_safe(self, config_name: str, required_fields: Optional[List[str]] = None) -> Dict[str, Any]:
        """
        ...
        if external_path.exists():
            try:
                config_data = self._load_and_validate_file(external_path, config_name, required_fields)
                ...
                if container_path.exists():
                    logger.info(f"🔧 Auto-repairing: replacing corrupted {external_path} with container default")
                    try:
        ...
        if container_path.exists():
            try:
                config_data = self._load_and_validate_file(container_path, config_name, required_fields)
    ...
    def _load_and_validate_file(self, file_path: Path, config_name: str, 
