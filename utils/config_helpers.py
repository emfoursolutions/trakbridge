...
def config_get_typed(config: Dict[str, Any], key: str, expected_type: Type[T], default: T) -> T:
    ...
    try:
        ...
        logger.warning(f"Failed to convert config value {key}={value} to {expected_type.__name__}: {e}")
        return default


def get_environment_config(config: Dict[str, Any], env_var_prefix: str = "TRAKBRIDGE_") -> Dict[str, Any]:
    """
...
def validate_config_keys(config: Dict[str, Any], required_keys: List[str], section_name: str = "config") -> List[str]:
    """
